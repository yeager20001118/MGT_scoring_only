...
def load_mask_model(model_name, device, cache_dir, hf_token):
    model_name = get_model_fullname(model_name)
    # mask filling t5 model
    print(f'Loading mask filling model {model_name}...')
    mask_model = from_pretrained(AutoModelForSeq2SeqLM, model_name, {}, cache_dir, hf_token)
    mask_model = mask_model.to(device)
    return mask_model

def load_mask_tokenizer(model_name, max_length, cache_dir,hf_token):
    model_name = get_model_fullname(model_name)
    tokenizer = from_pretrained(AutoTokenizer, model_name, {'model_max_length': max_length}, cache_dir, hf_token)
    return tokenizer
...
def generate_perturbs(args):
    ...
    # load model
    mask_model = load_mask_model(args.mask_filling_model_name, args.device, args.cache_dir, args.hf_token)
    mask_model.eval()
    try:
        n_positions = mask_model.config.n_positions
    except AttributeError:
        n_positions = 512
    mask_tokenizer = load_mask_tokenizer(args.mask_filling_model_name, n_positions, args.cache_dir, args.hf_token)
...
def experiment(args):
    ...
    scoring_model = load_model(args.scoring_model_name, 'cpu', args.cache_dir, args.hf_token)
    scoring_model.eval()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask_top_p', type=float, default=0.96)
    ...
    parser.add_argument('--hf_token', type=str, required=True, help="Hugging Face API token for accessing gated models")
